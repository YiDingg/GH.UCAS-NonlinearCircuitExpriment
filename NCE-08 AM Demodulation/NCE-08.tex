...
%%%%%%%%%%%%%%%%%% 1. 修改页眉内容 %%%%%%%%%%%%%%%%%%
\rhead{\small NCE-08 AM Demodulation (2025.12.11, 丁毅)}
...
\section{实验原理}
\subsection{同步幅度解调基本原理 (Synchronous AM Demodulation)}
...
\begin{enumerate}
    ...
    \item \textbf{不同调制系数下的解调}：在幅度调制板中，依次将调制系数 $m$ 调节至约 0.3、0.5、0.8，对每一个 $m$ 值，在解调板上记录 TP3 和 TP4 的波形；对比不同 $m$ 下，解调输出波形 (TP4) 的幅度和噪声变化。
\end{enumerate}

\section{实验结果与分析}

为解调电路设置合适工作点之后，调整幅度调制电路使输出法信号调幅系数约为 0.5，此时的调幅结果与解调结果如 Figure \ref{fig__demod_result_moderate} 所示：


\begin{figure}[H]\centering
\begin{subfigure}[b]{0.48\columnwidth}\centering
    \includegraphics[height=140pt]{NCE-08 AM Demodulation/assets/NCE-08_调制系数适中时，解调输出质量较好 (无明显失真).png}
    \caption{Amplitude-Modulated signal (CH1, blue) and filtered demodulated signal (CH2, red).}
\end{subfigure}\hfill
\begin{subfigure}[b]{0.48\columnwidth}\centering
    \includegraphics[height=140pt]{NCE-08 AM Demodulation/assets/NCE-08_调制系数适中时，解调输出质量较好 (无明显失真) (两个输出).png}
    \caption{Unfiltered (CH1, blue) demodulated signal and filtered (CH2, red) demodulated signal.}
\end{subfigure}
\caption{The measured AM modulation and demodulation results with moderate modulation index {\color{red} $(m \approx 0.5)$}.}
\label{fig__demod_result_moderate}
\end{figure}

改变调幅系数后继续进行测量，结果如 Figure \ref{fig__demod_result_small} 和 Figure \ref{fig__demod_result_large} 所示：

\begin{figure}[H]\centering
\begin{subfigure}[b]{0.48\columnwidth}\centering
    \includegraphics[height=140pt]{NCE-08 AM Demodulation/assets/NCE-08_调制系数较小时，解调输出幅度较小，但线性度更高.png}
    \caption{Amplitude-Modulated signal (CH1, blue) and filtered demodulated signal (CH2, red).}
\end{subfigure}\hfill
\begin{subfigure}[b]{0.48\columnwidth}\centering
    \includegraphics[height=140pt]{NCE-08 AM Demodulation/assets/NCE-08_调制系数较小时，解调输出幅度较小，但线性度更高 (两个输出).png}
    \caption{Unfiltered (CH1, blue) demodulated signal and filtered (CH2, red) demodulated signal.}
\end{subfigure}
\caption{The measured AM modulation and demodulation results with small modulation index {\color{red} $(m \approx 0.2)$}.}
\label{fig__demod_result_small}
\end{figure}

\begin{figure}[H]\centering
\begin{subfigure}[b]{0.48\columnwidth}\centering
    \includegraphics[height=140pt]{NCE-08 AM Demodulation/assets/NCE-08_调制系数过大时，解调输出出现明显失真.png}
    \caption{Amplitude-Modulated signal (CH1, blue) and filtered demodulated signal (CH2, red).}
\end{subfigure}\hfill
\begin{subfigure}[b]{0.48\columnwidth}\centering
    \includegraphics[height=140pt]{NCE-08 AM Demodulation/assets/NCE-08_调制系数过大时，解调输出出现明显失真 (两个输出).png}
    \caption{Unfiltered (CH1, blue) demodulated signal and filtered (CH2, red) demodulated signal.}
\end{subfigure}
\caption{The measured AM modulation and demodulation results with large modulation index {\color{red} $(m > 1)$}.}
\label{fig__demod_result_large}
\end{figure}



可以看到，经过低通滤波后的输出波形更加“纯净”，波形上的高频毛刺大多被消除，即附在信号上的高频分量基本被滤除。需要指出的是，通过低通滤波前的解调信号应该还含有直流分量，但我们测量时将示波器设置为了交流耦合，因此没有观察到直流分量。



在不同的调幅系数下信号均能被解调，但调幅系数过大或过小都会导致解调输出的质量下降。在调幅系数 $m > 1$ 时，解调输出出现了明显失真，波形顶部被削平 (Clipping)；而在调幅系数较小时，解调输出的幅度较小，信噪比较低.

上图还可以观察到，解调后的信号经过低通滤波后，高频毛刺明显减少，信号更加纯净。
...
\subsection{为什么解调信号在经过滤波后出现了“反相” (180° 相移)？}

查看原理图发现，解调电路中的滤波部分内含一个 Common Emitter (CE) 放大器，放大解调信号在滤波后经过 CE 放大器，因此出现了 180° 的相移。
...
注：本次实验并未产生具体数据，仅通过示波器观察波形变化，已在正文部分给出相关图像，此处不再重复记录。
\end{graybox}
...
\vspace*{\fill}
\thispagestyle{fancy} 
\includepdf[pages={-}]{NCE-08 AM Demodulation/preview/NCE-08 (preview report).pdf}

\section*{附录 C \hspace*{20pt} MATLAB Codes}
\addcontentsline{toc}{section}{附录 C \hspace*{6pt} MATLAB Codes} 
\thispagestyle{fancy} 
\lstinputlisting{d:/a_RemoteRepo/GH.MatlabCodes/本科课程代码/Non-Linear Circuits/NCE_08.m}
